        ...
        visitor: {
            Program: {
                enter(path, state) {
                    ...
                    if (!regex.some((re) => filename.match(re)) ||
                    ...
                    state.stylesheet = t.variableDeclaration("var", [
                        t.variableDeclarator(state.stylesheetId, t.callExpression(t.identifier("require"), [t.stringLiteral(options.css)])),
                    ]);
                    state.getStyleId = path.scope.generateUidIdentifier();
                    state.getStyle = t.variableDeclaration("var", [
                        t.variableDeclarator(state.getStyleId, t.memberExpression(t.memberExpression(t.callExpression(t.identifier("require"), [
                            t.stringLiteral(nodePath.join(libRoot, "./transformer-runtime")),
                        ]), t.identifier("default")), t.identifier("getStyle"))),
                    ]);
                },
                exit(path, state) {
                    if (!state.enabled)
                        return;
                    if (!state.cssTransformed)
                        return;
                    ...
                    [state.stylesheet, state.getStyle]
                        .reverse()
                        .forEach((node) => lastImport ? lastImport.insertAfter(node) : path.unshiftContainer("body", node));
                },
            },
            JSXElement(path, state) {
                ...
                const propInheritStyle = t.logicalExpression("&&", t.memberExpression(t.identifier("arguments"), t.numericLiteral(0), true), t.memberExpression(t.memberExpression(t.identifier("arguments"), t.numericLiteral(0), true), t.identifier("inheritStyle")));
                ...
                if (!inheritStyle && !className && !style)
                    return;
