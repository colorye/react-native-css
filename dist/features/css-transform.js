...
export default function CssTransform() {
    ...
    this.isPropertySupported = (property, value) => {
        if (UNSUPPORTED_PROPERTIES.includes(property))
            return false;
        if (value === undefined)
            return false;
    ...
    this.transformTransform = (property, value) => {
        const transformRe = /(perspective|rotate|rotateX|rotateY|scale|scaleX|scaleY|translate|translateX|translateY|skew|skewX|skewY)\s*\(\s*([^,)]+)[,\s]*([^)]+)?\)/g;
        const transforms = [];
        let match;
        do {
