...
function STYLE_FACTORY(props) {
    ...
    if (value === undefined || !(regex instanceof RegExp))
        return;
    if (regex.test(value)) {
...
function SPACING(value) {
    ...
    return STYLE_FACTORY({
        value,
        regex,
        getter: (spacings) => spacings.map((spacing) => {
            if (["auto", "%", "vw", "vh"].some((x) => spacing.includes(x)))
                return spacing;
...
function FONT_WEIGHT(value) {
    ...
    if (Array.isArray(fontWeightStyle))
        return fontWeightStyle[0];
    return fontWeightStyle;
}
function TRANSFORM(value) {
    const regex = /(\w+|-?\d+(deg)?%?)/g;
    const transformTokenRegex = /^(perspective|rotate|rotateX|rotateY|scale|scaleX|scaleY|translate|translateX|translateY|skew|skewX|skewY)$/g;
    if (!value)
        return;
    ...
    return transforms
        ...
        if (isTransformToken) {
            if (currentTransform)
                res.splice(-1, 1, ...parseTransform(currentTransform));
            res.push([token]);
        }
        else {
            if (currentTransform)
                currentTransform.push(token);
        }
        if (isLast) {
